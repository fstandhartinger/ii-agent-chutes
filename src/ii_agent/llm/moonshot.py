...
class MoonshotDirectClient(LLMClient):
    """Use Kimi/Moonshot models via Anthropic-compatible API."""
    ...
    def __init__(
        ...
        """Initialize the Moonshot client with Anthropic-compatible API."""
        # Get Moonshot API key
        api_key = os.getenv("MOONSHOT_API_KEY")
        if not api_key:
            raise ValueError("MOONSHOT_API_KEY environment variable must be set")
        
        # Create Anthropic client but with Moonshot base URL
        ...
        # Log provider info
        logging.info(f"Using Moonshot LLM provider with model: {model_name}")
    ...
    def generate(
        ...
        """
        # Log each LLM call
        logging.info(f"[MOONSHOT LLM CALL] model={self.model_name}, max_tokens={max_tokens}, temperature={temperature}")

        # Turn GeneralContentBlock into Anthropic message format (same as AnthropicDirectClient)
        anthropic_messages = []
        ...
        for retry in range(self.max_retries):
            try:
                ...
                if retry == self.max_retries - 1:
                    print(f"Failed Moonshot request after {retry + 1} retries")
                    raise e
                else:
                    print(f"Retrying Moonshot LLM request: {retry + 1}/{self.max_retries}")
                    # Sleep 12-18 seconds with jitter to avoid thundering herd.
                    time.sleep(15 * random.uniform(0.8, 1.2))
            except Exception as e:
                raise e

        # Convert messages back to internal format (same as AnthropicDirectClient)
        internal_messages = []
